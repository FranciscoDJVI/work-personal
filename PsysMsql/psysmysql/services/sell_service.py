...
class SellService:
    ...
    def calculate_sell_totals(sell_products_queryset):
        ...
        with LogOperation(f'Calculando totales para {items_count} productos', logger):
            ...
            # Cálculos de IVA
            iva_amount = subtotal * SellService.IVA_RATE
            price_without_iva = subtotal - iva_amount
            
            logger.info(f'Totales calculados: {items_count} productos, subtotal: {subtotal}, IVA: {iva_amount}')
    ...
    def add_product_to_sell(product_id, quantity):
        ...
        try:
            with LogOperation(f'Agregando producto {product_id} con cantidad {quantity}', logger):
                product = Products.objects.get(pk=product_id)
                
                # Crear o actualizar el item en SellProducts
                sell_product, created = SellProducts.objects.get_or_create(
                    idproduct=product,
                    # Aquí podrías tener lógica para asociar a una venta específica
                    defaults={
                        'quantity': quantity,
                        'priceunitaty': product.price
                    }
                )
                ...
                if not created:
                    ...
                    logger.info(f'Nuevo producto {product.name} agregado al carrito: {quantity} unidades')
    ...
    def remove_sell_item(item_id):
        ...
        try:
            with LogOperation(f'Eliminando item {item_id} del carrito', logger):
                ...
                logger.info(f'Item eliminado del carrito: {product_name} ({quantity} unidades)')
                return True
    ...
    def calculate_change(total_amount, payment_amount):
        ...
        try:
            ...
            logger.info(f'Cambio calculado: total=${total}, pago=${payment}, cambio=${change}')
            
            return change
            
        except Exception as e:
            logger.error(f'Error calculando cambio: total={total_amount}, pago={payment_amount}, error={str(e)}')
            raise
    
    @staticmethod
    @log_function_call(get_sell_logger())
    def create_sell_register(employee_id, total_sell, type_pay, state_sell, 
                           notes, quantity_pay, detail_sell):
        """
        ...
        try:
            with LogOperation(f'Creando registro de venta: total=${total_sell}', logger):
                ...
                logger.info(f'Venta registrada exitosamente: ID={sell_register.pk}, empleado={employee_id}, total=${total_sell}, tipo_pago={type_pay}')
                return sell_register
                
        except Exception as e:
            logger.error(f'Error creando registro de venta: empleado={employee_id}, total={total_sell}, error={str(e)}')
            raise
    ...
    def get_sell_summary_for_template(request):
        ...
        with LogOperation('Obteniendo resumen de venta para template', logger):
            ...
            logger.info(f'Resumen de venta preparado: {products_count} productos, subtotal={totals.get("subtotal", 0)}')
    ...
    def process_complete_sale(employee_id, payment_amount, payment_type='efectivo', 
        ...
        try:
            with LogOperation(f'Procesando venta completa: empleado={employee_id}, pago=${payment_amount}', logger):
                ...
                if payment_amount < total_amount:
                    logger.error(f'Pago insuficiente: necesario=${total_amount}, recibido=${payment_amount}')
                ...
                logger.info(f'Venta procesada exitosamente: ID={sell_register.pk}, cambio=${change}')
    ...
    def get_sales_statistics(date_from=None, date_to=None):
        ...
        try:
            with LogOperation(f'Obteniendo estadísticas de ventas: {date_from} a {date_to}', logger):
                ...
                payment_types = queryset.values('type_pay').annotate(
            ...
            with LogOperation('Validando carrito antes del checkout', logger):
                ...
                for item in sell_products:
                    ...
                    if abs(float(current_price - item.priceunitaty)) > 0.01:
                        warnings.append(f'Precio de {item.idproduct.name} ha cambiado: ${item.priceunitaty} -> ${current_price}')
                ...
                if result['valid']:
                    logger.info(f'Carrito validado exitosamente: {result["products_count"]} productos')
                else:
                    logger.warning(f'Carrito inválido: {len(errors)} errores, {len(warnings)} advertencias')
