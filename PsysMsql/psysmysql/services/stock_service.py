...
class StockService:
    ...
    def get_stock_summary():
        ...
        with LogOperation('Generando resumen de stock', logger):
            ...
            stock_data = Stock.objects.select_related('id_products').aggregate(
                ...
                low_stock_count=Sum(
                    1, filter=Q(quantitystock__lt=10)
                ),
                out_of_stock_count=Sum(
                    1, filter=Q(quantitystock=0)
                )
            )
            ...
            # Productos sin stock
            out_of_stock_products = Stock.objects.select_related('id_products').filter(
                quantitystock=0
            ...
            logger.info(f"Resumen de stock generado: {summary['total_products_in_stock']} productos")
            return summary
    ...
    def update_stock(product_id, quantity, operation='set'):
        ...
        with LogOperation(f'Actualizando stock producto {product_id}: {operation} {quantity}', logger):
            try:
            ...
            if operation == 'add':
                ...
                if stock.quantitystock < quantity:
                    logger.warning(f"Intento de reducir stock por debajo de 0 para producto {product_id}")
                    raise ValidationError(f"Stock insuficiente. Disponible: {stock.quantitystock}, Solicitado: {quantity}")
                stock.quantitystock = F('quantitystock') - quantity
            elif operation == 'set':
                stock.quantitystock = quantity
            else:
                raise ValidationError("Operación inválida. Use 'add', 'subtract' o 'set'")
            ...
            logger.info(f"Stock actualizado para producto {product.name}: {stock.quantitystock}")
    ...
    def check_stock_availability(product_id, required_quantity):
        ...
        try:
            ...
            if not available:
                logger.warning(f"Stock insuficiente para producto {product_id}: disponible {stock.quantitystock}, requerido {required_quantity}")
            
            return {
                'available': available,
                'current_stock': stock.quantitystock,
                'required': required_quantity,
                'remaining_after': stock.quantitystock - required_quantity if available else None
            }
    ...
    def get_stock_alerts():
        ...
        with LogOperation('Generando alertas de stock', logger):
            ...
            # Productos sin stock
            out_of_stock = Stock.objects.select_related('id_products').filter(
                quantitystock=0
            ).count()
            ...
            low_stock = Stock.objects.select_related('id_products').filter(
                quantitystock__gt=0,
                quantitystock__lt=10
            ).count()
    ...
    def bulk_update_stock(stock_updates):
        ...
        with LogOperation(f'Actualización masiva de stock: {len(stock_updates)} productos', logger):
            results = []
            errors = []
            ...
            logger.info(f"Actualización masiva completada: {len(results)} exitosos, {len(errors)} errores")
    ...
    def get_stock_movements_report(days=30):
        ...
        with LogOperation(f'Generando reporte de movimientos ({days} días)', logger):
            ...
            recent_movements = SellProducts.objects.select_related(
                'idproduct'
            ).filter(
    ...
    def suggest_restock_quantities():
        ...
        with LogOperation('Generando sugerencias de reposición', logger):
            ...
            low_stock_items = Stock.objects.select_related('id_products').filter(
                quantitystock__lt=10
