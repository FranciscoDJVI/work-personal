"""
Configuración de logging para el proyecto PsysMsql
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(
                os.path.join(log_dir, f'psysmysql_{datetime.now().strftime("%Y%m%d")}.log')
            ),
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                execution_time = time.time() - start_time
                func_logger.error(f"{func_name} falló después de {execution_time:.3f}s: {str(e)}")
                raise
...
class LogOperation:
    """
    Context manager para logging de operaciones complejas
    
    Usage:
        with LogOperation('Creating new product', get_product_logger()):
            # ... operación ...
    """
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            ...
            self.logger.error(f"Error en: {self.operation_name} ({duration:.3f}s): {str(exc_val)}")
