...
class StockViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = Stock.objects.all().select_related('name')
    serializer_class = StockSerializer
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_class = StockFilter
    search_fields = ['name__name']
    ordering_fields = ['quantity', 'name__name']
    ordering = ['name__name']
    ...
    def summary(self):
        ...
        total_stock_value = sum(
            stock.quantity * stock.name.price 
            for stock in queryset
        )
        low_stock_count = sum(
            1 for stock in queryset 
...
class ClientViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    search_fields = ['name', 'lastname', 'email', 'telephone']
    ordering_fields = ['name', 'lastname', 'email']
    ordering = ['name']
    ...
    def stats(self):
        ...
        top_products = (
            SellProducts.objects
            .filter(sell__client=client)
            .values('name__name')
        ...
        data = {
            ...
            'first_purchase': sells.order_by('created_at').first().created_at if sells.exists() else None,
            'last_purchase': sells.order_by('-created_at').first().created_at if sells.exists() else None
        }
...
class SellViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = Sell.objects.all().select_related('client', 'user').prefetch_related('sellproducts_set__name')
    serializer_class = SellSerializer
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_class = SellFilter
    search_fields = ['client__name', 'client__lastname', 'user__username']
    ...
    def analytics(self, request):
        ...
        top_products_data = (
            SellProducts.objects
            .filter(sell__in=sells_queryset)
            .values('name')
        ...
        data = {
            ...
            'sales_by_payment_method': {item['payment_method']: item['total'] for item in payment_methods},
