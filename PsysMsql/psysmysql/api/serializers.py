...
class ProductSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        ...
        if self.instance:
            ...
            if (
                Products.objects.exclude(pk=self.instance.pk)
                .filter(name=value)
                .exists()
            ):
...
class ProductListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Products
        fields = ["idproducts", "name", "price", "stock_quantity", "stock_status"]

    @staticmethod
    def get_stock_quantity(obj):
        try:
            stock = Stock.objects.get(id_products=obj)
            return stock.quantitystock
        except Stock.DoesNotExist:
...
class StockSerializer(serializers.ModelSerializer):
    ...
    product_name = serializers.CharField(source="id_products.name", read_only=True)
    product_price = serializers.DecimalField(
        source="id_products.price", max_digits=10, decimal_places=2, read_only=True
    )
    product_id = serializers.IntegerField(
        source="id_products.id_products", read_only=True
    )
...
class ClientSerializer(serializers.ModelSerializer):
    ...
    def validate_email(self, value):
        ...
        if self.instance:
            if (
                Clients.objects.exclude(pk=self.instance.pk)
                .filter(email=value)
                .exists()
            ):
...
class SellProductSerializer(serializers.ModelSerializer):
    ...
    product_price = serializers.DecimalField(
        source="name.price", max_digits=10, decimal_places=2, read_only=True
    )
    subtotal = serializers.SerializerMethodField()

    class Meta:
        model = SellProducts
        fields = ["id", "name", "product_name", "product_price", "quantity", "subtotal"]
...
class SellSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Sell
        fields = ["idsell", "datesell", "totalsell", "id_product"]
        read_only_fields = ["idsell", "datesell", "totalsell", "id_product"]
    ...
    def validate_products(value):
        ...
        for product_data in value:
            if "product_id" not in product_data or "quantity" not in product_data:
                raise serializers.ValidationError(
                    "Cada producto debe tener product_id y quantity"
                )

            try:
                product = Products.objects.get(idproducts=product_data["product_id"])
                stock = Stock.objects.get(id_products=product.idproducts)
                ...
                if stock.quantitystock < quantity:
                    raise serializers.ValidationError(
                        f"Stock insuficiente para {product.name}. Disponible: {stock.quantitystock}"
                    )
            except Products.DoesNotExist:
                raise serializers.ValidationError(
                    f"Producto no encontrado: {product_data['product_id']}"
                )
            except Stock.DoesNotExist:
                raise serializers.ValidationError(
                    f"Stock no encontrado para producto: {product_data['product_id']}"
                )
            except ValueError:
...
class RegisterSellDetailSerializer(serializers.ModelSerializer):
    ...
    idsell = serializers.IntegerField(read_only=True)
    date = serializers.DateTimeField(read_only=True)
    id_employed = serializers.CharField(read_only=True)
    total_sell = serializers.DecimalField(
        max_digits=10, decimal_places=2, read_only=True
    )
    type_pay = serializers.CharField(read_only=True)
    state_sell = serializers.CharField(read_only=True)
    notes = serializers.CharField(read_only=True)
    quantity_pay = serializers.DecimalField(
        max_digits=10, decimal_places=2, read_only=True
    )
    detail_sell = serializers.CharField(read_only=True)
